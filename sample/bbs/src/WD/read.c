  ...
  char *msg;
{
  ...
  num = TagNum;
  sprintf(buf, "¡» %s (A)¤å³¹ (T)¼Ð°O (Q)¨S¨Æ¡H[%c] ", msg, num ? 'T' : 'A');
  switch (getans(buf))
  {
  case 'q':
    num = -1;
...
/* wildcat */
    ...
    case 'w':
      if (HAS_PERM(PERM_LOGINOK))
      {
        ...
        my_write(owneronline->pid, "¼ö½u Call-In¡G");
        return RC_FULL;
      }
      break;
    ...
    case 't':
      ...
      {
        if (gem_mode & GEM_TAG)
          pressanykey("¦p¤£«þ¨©ªO­±¤å³¹, ½Ð¥ý«ö Ctrl-C ±NªO­±¼ÐÅÒ§R°£");
    ...
    case 'l':	/* ±N '*' Âà¬° 't' */
      if (currmode & MODE_BOARD)
      {
        ...
        sprintf(genbuf, "(1)Âê©w * ¼Ð°O (2)¨Ï¥Î %c ¼Ð°O [Q]uit ?",
          (currmode & MODE_TAG) ? '*' : 't');
	 ...
	 num_record = NULL;
      ...
      if (mode == RC_NEWDIR)
      {
        if (last_line == 0)
        {
          ...
          {
            if (!gem_none())	/* shakalaca.000525: ¥Î i_read ªº¯ÊÂI.. :X
            			   ±o¥Î¥t¤@­Ó function ¼ÒÀÀ¥XªÅºëµØ°Ïªº¼Ë¤l.. :p */
              goto return_i_read;
          }                                                                
#endif
          else if (currstat == LISTMAIN)
          {
            getdata(1, 0, "©|¥¼¦³¦W³æ (N)·s¼W (Q)Â÷¶}¡H[Q] ",
              genbuf, 4, LCECHO, 0);
            if (genbuf[0] == 'n')
              list_add();
            goto return_i_read;
