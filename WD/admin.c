...
m_newbrd ()
{
  ...
  newboard.title[4] = ' ';
  getdata (7, 0, "1.Âà«H ¡´ 2.¤£Âà«H ¡· 3.¤l¿ï³æ £U 4.¤ÀÃþ¬ÝªO ¡¼", genbuf, 3
	   ,LCECHO, 0);
...
m_mod_board (char *bname)
{
  ...
  bperm_msg (&bh);
  prints ("%s³]­­  ÁÜ½Ð:%s\n"
	  "¥izap:%s ²Î­p:%s Âà«H:%s ¸s²ÕªO:%s ¨p¤H:%s ÁôÂÃ:%s °Î¦W:%s Àu¨}:%s ­Ó¤H:%s",
	  bh.level ? "¦³" : "¤£",
	  bh.brdattr & BRD_INVITE ? "£¾" : "¢æ",
  ...
  switch (*genbuf)
	...
	strncpy (newbh.title, genbuf, 4);
      ...
      newbh.title[4] = ' ';
      getdata (13, 0, "[1.Âà«H ¡´] [2.¤£Âà«H ¡·] [3.¤l¿ï³æ £U] [4.¤ÀÃþ¬ÝªO ¡¼]", genbuf, 3
	       ,LCECHO, 0);
      if (genbuf[0])
	{
      ...
      if (!(newbh.brdattr & BRD_GROUPBOARD || newbh.brdattr & BRD_CLASS))
	{
	  char ans[4];
          ...
          if (*ans == 'y' || *ans == 'Y')
            ...
            newbh.brdattr &= ~BRD_PERSONAL;
	  getdata (19, 0, HAS_PERM (PERM_SYSOP) ? "¬ÝªOÁôÂÃ/¨p¤H/¤½¶}/ÁÜ½Ð(Y/P/N/I)¡H"
