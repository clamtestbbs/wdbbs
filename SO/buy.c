...
p_cloak()
{
  char buf[4];
  getdata(b_lines-1, 0,
    currutmp->invisible ? "½T©w­n²{¨­?[y/N]" : "½T©w­nÁô¨­?[y/N]",buf,3,LCECHO,"N");
  if(buf[0]!='y') 
    return;
...
p_fcloak()
{
  ...
  getdata(b_lines-2, 0, "½T©w­nªá $500 ¾Ç²ß²×·¥Áô¨­¤jªk¡H[y/N]",ans,3,LCECHO,0);
  if(ans[0]!='y')
    return;
...
p_from()
{
  ...
  if(ans[0]!='y') return;
  if (getdata(b_lines-1, 0, "½Ğ¿é¤J·s¬G¶m:", currutmp->from, 17, DOECHO,0))
        {
           degold(5);
...
p_ffrom()
{
  ...
  getdata(b_lines-2, 0, "½T©w­nªá $1000 ÁÊ¶R­×§ï¬G¶mÄ_¨å¡H[y/N]",ans,3,LCECHO,0);
  if(ans[0]!='y') return;
    rec_get(fn_passwd, &xuser, sizeof(xuser), usernum);
...
exchange()
{
  ...
  outs("\nª÷¹ô ¡G »È¹ô  =  1 ¡G 10000\n");
  if(!getdata(17,0,"(1)»È¹ô´«ª÷¹ô  (2)ª÷¹ô´«»È¹ô ",ans,3,LCECHO,0)) return;
